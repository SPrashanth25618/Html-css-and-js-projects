...
function makePassword(){
  ...
  if(!includeNumbers && !includeLowercase && !includeSymbols && !includeUppercase){
...
function createRandomPassword(length,includeUppercase,includeLowercase,includeNumbers,includeSymbols){
  ...
  for(let i=0;i<length;i++){
    const ranIdx = Math.floor(Math.random()*allCharacters.length);
    password += allCharacters[ranIdx];
  }
  return password;
}
